<%-- 
    Document   : userdata
    Created on : Mar 19, 2021, 10:28:44 AM
    Author     : Admin
--%>
...
<html>
    ...
    <div style="color: white; text-align: center; font-size: 30px;">All User Data </div>
<%
                            ...
                            %>
                             ...
                             <%
                            String removermsg=(String)session.getAttribute("removed");
                            if(removermsg!=null)
                            {%>
                               <h3 class="alert alert-danger"><%=removermsg %></h3>
                            <%
    ...
    <table>
        ...
        <thead>
            <%
                ...
                try {
                    ...
                    ResultSet rs1 = st1.executeQuery("select *from userdata where 1 ");
                    while (rs1.next()) {
        ...
        <tbody>
            <%                ResultSet rs = st1.executeQuery("select * from userdata where 1");
                while (rs.next()) {
            %>
            <tr>
                <%sno = sno + 1; %>
                ...
                <td><a href="edituserdata.jsp?id=<%=rs.getString(1)%>">Edit <i class='glyphicon glyphicon-edit'></i></a></td>
                <td><button type="button" class="btn btn-success"><a href="deletuserdata.jsp?id=<%=rs.getString(1)%>">Remove <i class='fas fa-trash-alt'></i></a></button></td>
